...
final class Date {
	...
	public static function fromString ($s) {
		...
		if ($__hx__switch === 8) {
			...
			$k1 = HxString::split($s, "-");
			$tmp = \Std::parseInt(($k1->arr[0] ?? null));
			$tmp1 = \Std::parseInt(($k1->arr[1] ?? null)) - 1;
			return new Date($tmp, $tmp1, \Std::parseInt(($k1->arr[2] ?? null)), 0, 0, 0);
		} else if ($__hx__switch === 19) {
			$k2 = HxString::split($s, " ");
			$y = HxString::split(($k2->arr[0] ?? null), "-");
			$t = HxString::split(($k2->arr[1] ?? null), ":");
			$tmp2 = \Std::parseInt(($y->arr[0] ?? null));
			$tmp3 = \Std::parseInt(($y->arr[1] ?? null)) - 1;
			$tmp4 = \Std::parseInt(($y->arr[2] ?? null));
			$tmp5 = \Std::parseInt(($t->arr[0] ?? null));
			$tmp6 = \Std::parseInt(($t->arr[1] ?? null));
			return new Date($tmp2, $tmp3, $tmp4, $tmp5, $tmp6, \Std::parseInt(($t->arr[2] ?? null)));
		} else {
			throw new HxException("Invalid date format : " . ($s??'null'));
		}
	}
